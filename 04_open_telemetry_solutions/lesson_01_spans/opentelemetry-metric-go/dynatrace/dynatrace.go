	...
	err := cs.ForEach(e, func(r export.Record) error {
		...
		case aggregation.MinMaxSumCount:
			...
			case number.Float64Kind:
				lines = append(lines, mint.SerializeRecord(descriptor, mint.SerializeDoubleSummaryValue(min.AsFloat64(), max.AsFloat64(), sum.AsFloat64(), int64(count))))
			case number.Int64Kind:
				lines = append(lines, mint.SerializeRecord(descriptor, mint.SerializeIntSummaryValue(min.AsInt64(), max.AsInt64(), sum.AsInt64(), int64(count))))
			}
		...
		case *exact.Aggregator:
			points, err := agg.Points()
			if err != nil {
				return fmt.Errorf("error getting Points for %s: %w", descriptor, err)
			}
			for _, point := range points {
				lines = append(lines, mint.SerializeRecord(descriptor, fmt.Sprintf("%d", point.Number)))
			}
		}
